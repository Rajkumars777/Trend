...
export default function RealtimePage() {
    ...
    return (
        <div className="space-y-10 pb-12 w-full px-6">
            ...
            {/* Filter Toolbar */}
            <div className="bg-white/80 dark:bg-slate-900/60 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-2xl p-2 flex flex-wrap gap-3 items-center shadow-lg dark:shadow-2xl sticky top-4 z-40 transition-colors">
                <div className="flex items-center gap-2 text-slate-700 dark:text-slate-300 font-bold px-4 py-2 border-r border-slate-200 dark:border-slate-800 mr-2">
                ...
                <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2.5 hover:border-slate-300 dark:hover:border-slate-700 transition-colors">
                    <Calendar size={16} className="text-slate-500" />
                    <input
                        type="date"
                        value={filters.startDate}
                        onChange={(e) => handleFilterChange('startDate', e.target.value)}
                        className="bg-transparent text-sm text-slate-900 dark:text-slate-300 focus:outline-none w-32 dark:[color-scheme:dark]"
                    />
                ...
                {(filters.platform !== 'All' || filters.sentiment !== 'All' || filters.startDate) && (
                    <button
                        ...
                        className="text-xs text-red-500 dark:text-red-400 hover:text-red-600 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-500/10 px-4 py-2 rounded-xl transition-all ml-auto font-bold border border-transparent hover:border-red-200 dark:hover:border-red-500/20"
                    >
                        Reset
                    </button>
                )}
